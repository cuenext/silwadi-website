...
class PatchTenDoctorAuthority(unittest.TestCase):
    ...
    def test_every_profile_has_unique_metadata_breadcrumb_and_person_schema(self):
        titles=[]; descriptions=[]
        for slug,(name,specialty,image) in DOCTORS.items():
            rel=f'doctors/{slug}.html'; html=read(rel); canonical=f'https://silwadi.ae/{rel}'; title=re.search(r'<title>(.*?)</title>',html,re.I|re.S); desc=re.search(r'<meta\s+name="description"\s+content="([^"]+)"',html,re.I); self.assertIsNotNone(title,rel); self.assertIsNotNone(desc,rel); self.assertIn(name.replace('Dr. ',''),title.group(1),rel); self.assertIn('Abu Dhabi',title.group(1),rel); self.assertIn(f'<link rel="canonical" href="{canonical}">',html,rel); self.assertIn(f'<meta property="og:url" content="{canonical}">',html,rel); nodes=jsonld_nodes(rel); persons=[n for n in nodes if n.get('@type')=='Person']; crumbs=[n for n in nodes if n.get('@type')=='BreadcrumbList']; self.assertEqual(len(persons),1,rel); self.assertEqual(len(crumbs),1,rel); person=persons[0]; self.assertEqual(person['name'],name,rel); self.assertEqual(person['jobTitle'],specialty,rel); self.assertEqual(person['url'],canonical,rel); self.assertEqual(person['image'],f'https://silwadi.ae/assets/doctors/{image}',rel); self.assertEqual(person['worksFor']['@id'],'https://silwadi.ae/#dentist',rel); self.assertNotIn('award',json.dumps(person).lower(),rel); self.assertEqual(len(crumbs[0]['itemListElement']),3,rel); titles.append(title.group(1).strip()); descriptions.append(desc.group(1).strip())
    ...
    def test_low_confidence_profiles_stay_conservative(self):
        hawraa=read('doctors/dr-hawraa-al-ameri.html'); self.assertIn('Specialist Periodontist',hawraa); self.assertIn('periodontal',hawraa.lower()); self.assertNotRegex(hawraa,r'University|Master|Board|Fellow'); ahmed=read('doctors/dr-ahmed-el-shehri.html'); self.assertIn('root canal',ahmed.lower()); self.assertIn('minimally invasive',ahmed.lower()); self.assertNotIn('7,000',ahmed); self.assertNotIn('7000',ahmed); self.assertNotIn('Cairo University',ahmed); self.assertNotIn('Ajman University',ahmed)
