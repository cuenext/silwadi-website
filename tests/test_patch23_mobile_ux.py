...
class Patch23MobileUx(unittest.TestCase):
    ...
    def test_root_contact_footers_have_phone_and_email_icons(self):
        for path in ("index.html", "contact.html", "locations.html"):
            html = self.read(path)
            self.assertGreaterEqual(html.count('class="footer-contact-link"'), 2, path)
            self.assertGreaterEqual(html.count('class="contact-icon"'), 2, path)
